...
impl Transformer for TransformerService {
	...
	async fn basic_request(
		&self, request: Request<RequestObject>,
	...
	async fn request_stream(
		&self, request: Request<Streaming<RequestObject>>,
	) -> Result<Response<ResponseObject>, Status> {
	...
	async fn response_stream(
		...
		tokio::spawn(async move {
			for _ in 0..num_buffers {
				tx.send(Ok(ResponseObject { id: "response_stream".to_string() })).await.unwrap();
			}
		});
	...
	async fn bidirectional(
		...
		tokio::spawn(async move {
			while let Some(req_obj) = streamer.message().await.unwrap() {
				println!("{:?}", req_obj.id);
				tx.send(Ok(ResponseObject { id: format!("hello {}", req_obj.id) })).await.unwrap();
			}
		});
		Ok(Response::new(ReceiverStream::new(rx)))
	}
